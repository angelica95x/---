...
bot.command('fix', async (ctx) => {
  ...
  if (number.length < 7 || number.length > 22) {
...
function getMainMenuMarkup(userId) {
  ...
  if (isOwnerStatus || isAdminStatus) {
    keyboard.push([{ text: '𝙾𝚆𝙽𝙴𝚁 𝙼𝙴𝙽𝚄', callback_data: 'owner_menu' }]);
  }

  // Baris 2: Menu Fix (Menggantikan MENU WHATSAPP)
  keyboard.push([{ text: '𝙵𝙸𝚇 𝙼𝙴𝙽𝚄', callback_data: 'menu_fix' }]);
  
  // Baris 3: Menu More
  keyboard.push([{ text: '𝙼𝙴𝙽𝚄 𝙼𝙾𝚁𝙴', callback_data: 'menu_more' }]);

  // Baris 4: Developer
  keyboard.push([{ text: '𝙳𝙴𝚅𝙴𝙻𝙾𝙿𝙴𝚁', url: USERNAME_OWNER.replace('@', 'https://t.me/') }]);
...
async function sendFullStartMenu(ctx, sendAudio = true) { // Tambahkan parameter sendAudio
    ...
    const text = `<blockquote>ᴄɪᴀᴏ ғʀᴀᴛᴇʟʟᴏ @${ctx.from.username || 'User'}. sᴏɴᴏ ᴜɴ ʙᴏᴛ ᴅɪ ᴄᴏɴᴛʀᴏʟʟᴏ ᴅᴇʟʟᴀ ʙɪᴏɢʀᴀғɪᴀ ᴅɪ ᴡʜᴀᴛsᴀᴘᴘ sᴠɪʟᴜᴘᴘᴀᴛᴏ ᴅᴀ ᴢᴀᴋᴋ
    ...
    try {
        ...
        if (sendAudio && fs.existsSync(audioPath)) {
            await ctx.replyWithAudio(
                { source: audioPath },
                {
                    title: '.⋆♱ 𝚉𝙰𝙺𝙺 ?', // <-- FONT BARU
                    performer: 'ᯓ★  ᴄᴏʀʟᴇᴏɴᴇ', // <-- FONT BARU
                    caption: '𝙿𝚁𝙴𝙼𝙸𝙴𝚁 𝚃𝙴𝙰𝙼 𝚉𝙰𝙺𝙺', // <-- FONT BARU
                }
            );
        }
    } catch (error) {
...
© 𝚃𝙴𝙰𝙼 𝙿𝚁𝙸𝙼𝙸𝙴𝚁
          ...
          ...Markup.inlineKeyboard([
            [{ text: '𝙹𝙾𝙸𝙽 𝙲𝙷𝙰𝙽𝙽𝙴𝙻', url: `https://t.me/${VERIFICATION_CHANNEL_USERNAME.replace('@', '')}` }],
            [{ text: '𝙹𝙾𝙸𝙽 𝙶𝚁𝚄𝙱', url: `https://t.me/${VERIFICATION_GROUP_USERNAME.replace('@', '')}` }],
            [{ text: '𝚄𝙳𝙰𝙷 𝙹𝙾𝙸𝙽', callback_data: 'back_to_start' }]
          ]) 
        }
      );
  }
...
bot.on('callback_query', async (ctx) => {
  ...
  const backToStartButton = [{ text: '𝙱𝙰𝙲𝙺', callback_data: 'back_to_start' }];
...
async function startAll() {
  try {
    console.log('🚀 𝐒𝐓𝐀𝐑𝐓𝐈𝐍𝐆 𝐓𝐄𝐋𝐄𝐆𝐑𝐀𝐌 𝐁𝐎𝐓...');
    ...
    try {
      ...
      console.error('𝐆𝐀𝐆𝐀𝐋 𝐊𝐈𝐑𝐈𝐌 𝐍𝐎𝐓𝐈𝐅𝐈𝐊𝐀𝐒𝐈 𝐊𝐄 𝐎𝐖𝐍𝐄𝐑:', error);
    }
    
  } catch (error) {
    console.error('❌ 𝐆𝐀𝐆𝐀𝐋 𝐌𝐄𝐌𝐔𝐋𝐀𝐈 𝐁𝐎𝐓:', error);
    process.exit(1);
  }
}
